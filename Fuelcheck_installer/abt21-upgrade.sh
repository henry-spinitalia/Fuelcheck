...
# -----------------------------------------------------------------------------------------------------------
#
# ISSUE 1: Ogni tanto mi perdo /etc/sysconfig/{prod|err}info.d
# ISSUE 2: UBI non se sta zitto...
#
...
gprs_init_files()
{
    ...
    V_MD5="e867862b584e124ad2abdaa0270d6015"
...
# -----------------------------------------------------------------------------------------------------------
#    Funzione: gprs_check_config()
# Descrizione: Controllo la configurazione del modem
#   Parametri: Nessuno
#     Globali: V_GPRS_IPR
#              V_GPRS_IMSI
#              V_GPRS_IMEI
...
gprs_check_config()
{
    ...
    utl_log_data "gprs_check_config" "Caricamento dati dal modem"
    ...
    # A questo punto devo controllare il PIN
    #gprs_send_cmd "AT+CIMI" -> da errore senza PIN

}

# -----------------------------------------------------------------------------------------------------------
#    Funzione: gprs_init()
# Descrizione: Apro la porta seriale
#   Parametri: Nessuno
#     Globali: C_GPRS_DEV
# -----------------------------------------------------------------------------------------------------------
gprs_init()
...
gprs_off()
{
    ...
    then
        ...
        poff
        # TODO: attende la disattivazione
        sleep 2
    fi
...
backup_data()
{
  ...
  if [ -e /mnt2/etc/sysconfig/prodinfo.d ]; then
    ...
    cp /mnt2/etc/sysconfig/prodinfo.d /mnt/tmp/etc_sysconfig_prodinfo.d
  else
  ...
  if [ -e /mnt2/etc/sysconfig/errinfo.d ]; then
    ...
    cp /mnt2/etc/sysconfig/errinfo.d /mnt/tmp/etc_sysconfig_errinfo.d
  else
...
mount_root()
{
  ...
  if [ "$?" = "0" ]; then
    ...
    utl_log_data "mount_root" " ubiattach fail"
    change_led 1 1
    wait_and_reboot
  fi
  usleep 100000
  ...
  if [ "$?" = "0" ]; then
    ...
    utl_log_data "mount_root" " mount fail"
    change_led 1 1
    wait_and_reboot
  fi
  usleep 100000
...
umount_root()
{
  ...
  utl_log_data "mount_root" "Smonto la root partition"
...
mount_devices()
{
  ...
  # /var e /var_chrooted devono puntare in memoria
...
change_init()
{
    ...
    utl_log_data "change_init" "Changing init"

    # TODO: dovrebbe aggiungere le linee in un punto preciso
    # TODO: dovrebbe sostituire getty con un REPL Read Evaluate Print Loop

    cat <<- 'EOF' >> /mnt2/etc/inittab

2:2345:respawn:/usr/bin/screen -D -m /opt/fuelcheck_bl/fb_ignite
3:2345:respawn:/usr/bin/screen -D -m /opt/fuelcheck_fw/fc_mcp.sh

EOF

    if [ "$?" = "0" ]; then
        utl_log_data "change_init" " Adding init ok"
    else
        utl_log_error "change_init" " Adding init fail"
    fi

}

# Installa il firmware nell'unita' di controllo
# -----------------------------------------------------------------------------------------------------------
install_fw()
{

    # Inizio dell'installazione
    utl_log_data "install_fw" "Avvio installazione"
    ...
    # Ora posso
    utl_log_data "install_fw" "Aggiorno la configurazione del modem"
    mkdir /var/lock
    ...
    utl_log_data "install_fw" "Aggiorno il software"
    ...
    cp /mnt/pkg_update.sh /var/tmp/pkg_update.sh
    chroot /mnt2 /var/tmp/pkg_update.sh
    sleep 1
...
{
    # Aggiungo ignite!
    mkdir -p /opt/fuelcheck_bl
    cp /mnt/ignite /opt/fuelcheck_bl/fb_ignite_temp
    if [ "$?" = "0" ]; then
      utl_log_data "Saving ignite ok"
    else
      utl_log_data "Saving ignite fail"
      exit 1
    fi
}
...
installer_init()
{
    ...
    # Accendo il led verde, carico il modulo del watchdog, e monto tutte le partizioni
    utl_change_led 1 0
    modprobe pindog
    mount_devices
    ...
    # Inizializza il modem, ma ancora non posso connettermi
    gprs_init
    gprs_on
    gprs_check_config
    gprs_check_pin
